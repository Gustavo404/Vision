...
formatar_recv() {
  ...
  local nome_arquivo_sem_extensao=$(basename "$arquivo_entrada" | cut -f 1 -d '_')
  local nome_arquivo_sem_extensao=$(basename "$nome_arquivo_sem_extensao" | cut -f 1 -d '.')
  local arquivo_saida="${nome_arquivo_sem_extensao}_recv.txt"
...
formatar_olt_recv() {
  ...
  local nome_arquivo_sem_extensao=$(basename "$arquivo_entrada" | cut -f 1 -d '_')
  local nome_arquivo_sem_extensao=$(basename "$nome_arquivo_sem_extensao" | cut -f 1 -d '.')
  local arquivo_saida="${nome_arquivo_sem_extensao}_olt_recv.txt"
  
  if [ -f "$arquivo_entrada" ]; then
    # Realiza a substituição usando o sed
    sed "s/OLT RECV POWER : //g; s/ (Dbm)//g; s/(Dbm)//g; s/\t//g" "$arquivo_entrada" > "$arquivo_saida"
    echo -e "${GREEN}Sinal formatado com sucesso e salvo em '$arquivo_saida'${NC}"
  else
    echo -e "${RED}O arquivo '$arquivo_entrada' não existe.${NC}"
    exit 1
...
formatar_send() {
  ...
  local nome_arquivo_sem_extensao=$(basename "$arquivo_entrada" | cut -f 1 -d '_')
  local nome_arquivo_sem_extensao=$(basename "$nome_arquivo_sem_extensao" | cut -f 1 -d '.')
  local arquivo_saida="${nome_arquivo_sem_extensao}_send.txt"
  
  if [ -f "$arquivo_entrada" ]; then
    # Realiza a substituição usando o sed
    sed "s/SEND POWER   : //g; s/ (Dbm)//g; s/(Dbm)//g; s/\t//g" "$arquivo_entrada" > "$arquivo_saida"
    echo -e "${GREEN}Sinal formatado com sucesso e salvo em '$arquivo_saida'${NC}"
  else
    echo -e "${RED}O arquivo '$arquivo_entrada' não existe.${NC}"
    exit 1
...
while getopts "i:t:s:" opcao; do
  case "$opcao" in
    ...
    s)
      arquivo_input="$OPTARG"
      formatar_send "$arquivo_input"
      formatar_recv "$arquivo_input"
      formatar_olt_recv "$arquivo_input"
      ;;
    *)
